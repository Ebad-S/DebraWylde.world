...
// authoritative and overrides the year-plan single monthly value. For Y2/Y3
// the year-plan figure continues to apply. The previous engine behaviour of
// also adding the shared-costs "personal portion" to drawings has been
// removed to prevent double counting; shared costs now flow exclusively
// through the Personal Cash Flow outflow model.
export function calculateOwnerAdjustments(normalizedState) {
  ...
  timeline.months.forEach((period, index) => {
    ...
    if (ownerExtractsAsDrawings) {
      if (period.yearIndex === 1 && personalDrawingsMonthlyY1) {
